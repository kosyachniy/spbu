...
while True:
	...
	d = np.linalg.inv(jacobian(x)).dot(f)
	print(d, x.reshape(10,))
	if max(d)[0] < e:
		break
...
def newton(x_orig):
	...
	print("Время:", time.clock() - start_time, "секунд")
	print("Операций:", operations_count)
	print(x_start.T)
...
if __name__ == '__main__':
	...
	for i in range(6, 11, 2):
		print("С переходом на модифицированный метод на", i, "итерации:")
		auto_modif_newton(x, i)
		print("\n")

	for i in range(6, 11, 2):
		print("Гибридный метод, где матрица пересчитывается каждые", i, "итераций:")
		hybrid_newton(x, i)
		print("\n")